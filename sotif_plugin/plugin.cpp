/*
 * Special TeamSpeak 3 plugin for Slop3
 *
 *	Project -> configuration properties -> c/c++ -> code generation -> runtime library -> /MT (Multi-threaded)
 *
 * VÄLJ MULTI-THREADED FÖR ATT BAKA IN DE SYSTEM-DLLs SOM BEHÖVS
 *
 * Copyright (c) 2016 Sotif
 */
...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Slop3 Plugin";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Slop3 Plugin";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
const char* ts3plugin_description() {
	...
	static char* result = NULL;  /* Static variable so it's allocated only once */
	if (!result) {
		const wchar_t* name = L"Hey now!\n"
		"När du connectat till en server så kommer du få en röd ruta ifall du inte muteat din mic.\n"
		"Rutan försvinner när du mutear och kommer när du unmutear.\n"
		"Du kan ändra storlek på rutan hur du vill, den håller sedan den storleken tills du startar om TeamSpeak";
		if (wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Hey now!\n"
				"När du connectat till en server så kommer du få en röd ruta ifall du inte muteat din mic.\n"
				"Rutan försvinner när du mutear och kommer när du unmutear.\n"
				"Du kan ändra storlek på rutan hur du vill, den håller sedan den storleken tills du startar om TeamSpeak";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
	...
	/* WE DO NOT USE THIS, COMMENT IT OUT / sotif */
	/*
	char* name;
 ...
 *
 * TYDLIGEN SÅ MÅSTE DENNA VARA IMPLEMENTERAD, ANNARS KRASHAR DET NÄR PLUGINET LADDAS?!?!!? WTF PLS
 *
 */
...
void ts3plugin_onUpdateClientEvent(uint64 serverConnectionHandlerID, anyID clientID, anyID invokerID, const char* invokerName, const char* invokerUniqueIdentifier) {
	printf("Nu e vi i onUpdateClientEvent\n");
	int muted;
	if (ts3Functions.getClientSelfVariableAsInt(serverConnectionHandlerID, CLIENT_INPUT_MUTED, &muted) != ERROR_ok) {
		printf("Nu fick vi error sry\n");
		/* handle error */
	}
	if (muted == 1) {
		printf("Nu ar det muted\n");
		rs.hide();
	}
	else if (muted == 0) {
		printf("Nu ar det live\n");
		rs.makeRed();
	}
}
